...
def main():
    ...
    if args.multi_test is True:
        ...
        # 检查模型目录是否存在
        if not models_list_dir.exists():
            print(f"错误: 模型目录不存在: {models_list_dir}")
            return
        ...
        if len(models_list) == 0:
            print(f"错误: 在目录 {models_list_dir} 中未找到任何 .pkl 模型文件")
            return
    else:
        ...
        if not args.model_dir:
            print("错误: 未提供模型路径，请使用 --model_dir 参数指定模型文件")
            return

        # 检查模型文件是否存在
        if not os.path.isfile(args.model_dir):
            print(f"错误: 模型文件不存在: {args.model_dir}")
            return
    ...
    if args.evaluate is True:
        ...
        ########################### Evaluating part #################################
            ...
            for i in range(test_len):
                model_path = models_list[i]
                print(f"Attempting to load model: {model_path}")

                if not os.path.isfile(model_path):
                    print(f"False:model does not exist: {model_path}")
                    continue
                ...
                try:
                    ...
                    # Check if file is corrupted or format issue
                    try:
                        # Try to open file directly
                        with open(model_path, 'rb') as f:
                            content = f.read(100)  # read first 100 bytes
                            print(f"First 100 bytes of file: {content}")
                    except Exception as file_err:
                        print(f"Failed to open file: {file_err}")
                    continue

                test_model.eval()

                if args.dataset == 'KITTI':
                    errors, error_names, edge_rmse, boundary_f1 = validate(args, val_loader, test_model, logger, 'KITTI')
                elif args.dataset == 'NYU':
        ...
        if args.img_save is False:
            ...
            if len(models_list) == 0:
                print("Error: No available model file")
                return

            model_path = models_list[0]
            print(f"Attempting to load model for image saving: {model_path}")
            ...
            try:
                ...
                # Handle DataParallel key name issue
                if 'module.' not in list(state_dict.keys())[0]:
                    # If state dict does not contain 'module.' prefix but current model is DataParallel
                ...
                test_model.load_state_dict(state_dict)
                print("Successfully loaded model for image saving")
            except Exception as e:
                print(f"Failed to load model: {e}")
                return
        ...
        if args.img_save is True:
            ...
            for idx, (rgb_data, gt_data, gt_dense, filename) in enumerate(val_loader):
                ...
                folder_name_list = ['/output_depth', '/output_depth_cmap_gray', '/output_depth_cmap_jet',
                                    '/ground_truth', '/input_rgb', '/dense_gt', '/dense_gt_cmap_gray',
                                    '/dense_gt_cmap_jet']
                ...
                for img in img_arr:
                    ...
                    if folder_name == '/output_depth_cmap_gray' or folder_name == '/dense_gt_cmap_gray':
                        ...
                        img_org = img.cpu().detach().numpy().ast(np.float32)
